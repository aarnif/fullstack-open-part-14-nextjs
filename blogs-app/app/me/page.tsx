...
const MyPage = async () => {
  ...
  return (
    <div>
      <div
        className="border-b border-slate-300 dark:border-slate-700 mb-4 pb-4"
        data-testid="user-profile"
      >
        <h2 className="text-2xl font-bold mb-4">My Profile</h2>
        <p className="mb-2" data-testid="user-name">
          <span className="font-bold">Name:</span> {user.name}
        </p>
        <p className="mb-4" data-testid="user-username">
          <span className="font-bold">Username:</span> {user.username}
        </p>
      </div>
      <div
        className="border-b border-slate-300 dark:border-slate-700 mb-6 pb-8"
        data-testid="api-token-section"
      >
        <h2 className="text-2xl font-bold pt-2 mb-4">API Token</h2>
        <div
          className="max-w-120 bg-slate-100 dark:bg-slate-900 p-4 mb-4 rounded"
          data-testid="token-display"
        >
          <p className="font-medium mb-1">Current token:</p>
          <p
            className="rounded p-2 break-all bg-slate-200 dark:bg-slate-800"
            data-testid="api-token"
          >
            {user.token ? (
              user.token
            ) : (
              <em data-testid="no-token-message">No token generated yet</em>
            )}
          </p>
        </div>
        <Button onClick={generateToken} data-testid="generate-token-button">
          Generate New Token
        </Button>
      </div>
